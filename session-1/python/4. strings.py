...
bio_data = "Name: amirul Kayes, Age: 25, Country: Bangladesh"
data = bio_data.split(", ")
print(f"suspect name is {data[0].split(": ")[1]}")
print(f"suspect age is {data[1].split(": ")[1]}")
print(f"suspect nationality is {data[2].split(": ")[1]}")
...
print(f"suspect is {dicktionary["name"]},height is {dicktionary['height']} and color is {dicktionary['color']}")
...
for number in test_numbers:
    if number.startswith("0088"):
        print(f"Invalid phone number: {number}")
    elif re.fullmatch(pattern, number):
